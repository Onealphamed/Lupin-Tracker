...
# Bibliography tab. Schema: Therapies | Sr. No | Article header | Header of
# communication | Link | Bucket. The Therapies column forward-fills (only
# the first row of each therapy block carries the label).
TAB_TOC = os.environ.get("TOC_TAB", "TOC Bank")
...
@app.route("/api/toc-bank")
@login_required
def toc_bank():
    """Return the TOC Bank tab as therapy-grouped entries.

    Shape: {therapies: [{therapy, count, items: [{sr_no, header, link,
    comm_header, bucket}]}], total, updated_at}

    The Therapies column is forward-filled (the sheet only writes the
    label on the first row of each therapy block, like the main tab).
    Header columns are matched fuzzily so renaming "Sr. No" → "S.No"
    etc. doesn't break the view.
    """
    raw = _gviz_csv(TAB_TOC)
    if not raw or len(raw) < 2:
        return jsonify({"therapies": [], "total": 0,
                        "updated_at": datetime.utcnow().isoformat() + "Z"})
    ...
    for row in raw[1:]:
        ...
        if t:
            current_therapy = t
        article = g(c_article)
        # Skip rows that have neither an article nor a link — those are
        # blank padding rows the sheet picks up.
